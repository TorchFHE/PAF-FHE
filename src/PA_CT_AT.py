...
def generate_layer_input_data(model: nn.Module , layer_name_input, directory_path,  train_data_loader):
    if(not os.path.exists(directory_path)):
            os.mkdir(directory_path)
    data_type = "_input"
    key = layer_name_input
...
def generate_data_set(dirctory_path , layer_name_input, split_point):
    train_path = "train/"
    valid_path = "val/"
...
def data_collection(model_input, layer_name_input, valid_data_loader, train_data_loader, split_point, input_data_save_path):
    model = copy.deepcopy(model_input)
    validate(model, valid_data_loader)
    generate_layer_input_data(model,  layer_name_input, input_data_save_path, train_data_loader)
    generate_data_set(input_data_save_path ,  layer_name_input, split_point)

def CT_train(sign_type, sign_scale, scale_path, layer_name_input, sign_nest_dict, batch_size, input_data_dirctory, output_floder_suffix):
...
def CT_collection(model, sign_nest_dict, layer_name, sign_type, sign_scale, scale_path,  dataset, data_path):
    if(dataset == "cifar10" or dataset == "cifar100"):
        ...
        split_point = 900
        batch_size = 40
    data_collection(model_input = model, layer_name_input = layer_name, 
                    ...
                    split_point = split_point, input_data_save_path = data_path)
    CT_train(sign_type = sign_type, sign_scale = sign_scale, scale_path = scale_path, layer_name_input = layer_name,
             sign_nest_dict = sign_nest_dict, batch_size = batch_size, input_data_dirctory = data_path, output_floder_suffix = "dynamic")

def train_network(input_data_dirctory, model, sign_type, valid_data_loader, train_data_loader, start_layer_name, max_layer_counter, lr_c, lr_w, dataset):
    sign_type = sign_type
    ...
    for key in sign_nest_dict:
        ...
        CT_collection(model = copy.deepcopy(G_model), sign_nest_dict = sign_nest_dict, layer_name = key, sign_type = sign_type,
                      sign_scale = 0, scale_path = None,  dataset = dataset, data_path = input_data_dirctory)
...
if __name__ == "__main__":
    ...
    sub_working_directory = args.working_directory + "PA_CT_AT_" + args.sign_type + "/"
    if(not os.path.exists(sub_working_directory)):
        os.mkdir(sub_working_directory)

    train_network(model=model, sign_type = args.sign_type, start_layer_name = start_layer_name,
                  valid_data_loader = valid_data_loader, train_data_loader = train_data_loader,
                  max_layer_counter = args.max_counter, lr_c = lr_c, lr_w = lr_w, 
                  input_data_dirctory = sub_working_directory, dataset = args.dataset)
